...
class Leavequota_model extends CI_Model
{
	...
	public function get_stat($user_id,$year)
	{
		$this->db->select("LQID,LQ_LTID,LQQuota,LQUsedDay,LQUsedHour,LQRemainDay,LTName");
		$this->db->from($this->table);
		$this->db->join($this->table_leavetype,"LQ_LTID = LTID","left");
		$this->db->where("LQ_UserID",$user_id);
		$this->db->where("LQYear",$year);
		$query = $this->db->get();
		return $query;
	}
}
